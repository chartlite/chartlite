...
function SectionHeading({ id, kicker, title, blurb }: { id: string; kicker: string; title: string; blurb: string }) {
  return (
    <div id={id} className="mb-8 max-w-2xl scroll-mt-40">
    ...
    <FrameworkProvider>
      <header className="mb-10 max-w-3xl">
        ...
        <p className="mt-4 text-lg text-mist-500">
          ...
          performance — live, with copy-paste code for each framework. Flip the
          chart theme from the nav to see it all re-theme at once.
        </p>
      </header>

      {/* sticky section nav */}
      <nav className="sticky top-24 z-30 mb-12 -mx-2 flex flex-wrap gap-1 rounded-xl glass border-glow px-2 py-2">
        {SECTIONS.map((s) => (
          <a
            key={s.id}
            href={`#${s.id}`}
      ...
      {/* Chart types */}
      <SectionHeading id="types" kicker="Nine types" title="Every chart type" blurb="Line, bar, area, scatter, pie & donut, radial gauge, and combo — one API, one theme system." />
      <div className="grid grid-cols-1 gap-8">
        <ExampleCard
          ...
          code={{
            ...
            react: `<AreaChart data={data} curve="smooth" />`,
            vue: `<AreaChart :data="data" curve="smooth" />`,
            svelte: `<div use:chart={{ type: 'area', data, curve: 'smooth' }} />`,
          }}
        >
        ...
        <ExampleCard
          ...
          code={{
            ...
            react: `<RadialChart data={[{ x: 'Score', y: 74 }]} max={100} startAngle={-120} endAngle={120} />`,
          }}
        >
        ...
        <ExampleCard
          ...
          code={{
            vanilla: `new ScatterChart('#chart', { data }).render();`,
      ...
      {/* Data formats */}
      <div className="mt-20">
        <SectionHeading id="data" kicker="Flexible input" title="Four data formats" blurb="Pass whatever shape your data is already in — Chartlite normalizes all four." />
        <div className="grid grid-cols-1 gap-8">
          <ExampleCard
            title="DataPoint[] & number[]"
            description="The classic [{ x, y }] array, or a bare number[] for quick charts."
            code={{
              vanilla: `new LineChart('#chart', { data: [30, 45, 38, 52, 48] }).render();`,
      ...
      {/* Interactivity */}
      <div className="mt-20">
        <SectionHeading id="interactivity" kicker="Overlays" title="Reference lines & annotations" blurb="Draw thresholds, highlight regions, and annotate points — all declaratively." />
        <div className="grid grid-cols-1 gap-8">
          <ExampleCard
